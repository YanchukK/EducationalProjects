...
namespace AdvancedControl
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            string[] words = { "Ниссан", "АльСсСсфа Ромео", "БМВ"};
            var countOfLetters = words.SelectMany(f => f.Trim(' ').ToUpper()).Distinct().Count();
            ...
            var newarray = array.OrderByDescending(f => f%10).OrderBy(f => f - (f%10));
            ...
            Console.WriteLine(string.Join(Environment.NewLine,
                newBooks.GroupBy(f => f.Author).Select(f => f.Key + ", " + f.Count())));
        }
    }
}
